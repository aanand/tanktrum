...
class Mirv(session: Session, tank: server.Tank) extends Projectile(session, tank) {
  ...
  override def update(delta: Int) {
    ...
    if (session.isInstanceOf[server.Server]) {
...
class MirvCluster(session: Session, tank: server.Tank) extends Projectile(session, tank) {
